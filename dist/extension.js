...
function executeTool(tool) {
  ...
  switch (tool.function.name) {
    ...
    case "run_command": {
      const cwd = root;
      try {
        const stdout = (0, import_child_process.execSync)(args.command, { cwd, timeout: 3e4, encoding: "utf-8", maxBuffer: 1024 * 1024 });
...
${e.stderr || ""}`.trim();
    ...
    case "grep_search": {
      ...
      try {
        return (0, import_child_process.execSync)(`findstr /S /N /C:"${args.pattern.replace(/"/g, '\\"')}" ${include.includes("*") ? include : "*"}`, {
          cwd: root,
          timeout: 15e3,
          encoding: "utf-8",
          maxBuffer: 1024 * 1024
...
var OpenCodeClient = class {
  ...
  async ask(model, messages) {
    const apiMessages = messages.map((m) => {
      const msg2 = { role: m.role };
      if (m.content) {
        msg2.content = m.content;
      } else if (m.role === "tool") {
        msg2.content = m.content || "";
      } else {
        msg2.content = m.content || "";
      }
      if (m.tool_calls) msg2.tool_calls = m.tool_calls;
      if (m.tool_call_id) msg2.tool_call_id = m.tool_call_id;
      if (m.name) msg2.name = m.name;
      return msg2;
    });
    const body = {
      model,
  ...
  async agentLoop(model, messages, onToolCall) {
    let finalContent = "";
    while (true) {
      ...
      if (response.tool_calls && response.tool_calls.length > 0) {
        ...
        for (const tc of response.tool_calls) {
          ...
          try {
            ...
            if (result.length > 5e4) result = result.slice(0, 5e4) + "\n... (truncated)";
          } catch (e) {
...
// src/sidePanel.ts
var HISTORY_KEY = "opencodeConversation";
var MODELS = [
...
var SidePanelProvider = class {
  constructor(extensionUri, client, context) {
  ...
  async onReady() {
    ...
    const saved = this.context.globalState.get(HISTORY_KEY, []);
    if (saved.length > 0) {
      this.messages = saved;
...
function activate(context) {
  const apiKey = vscode2.workspace.getConfiguration("opencodeAgent").get("apiKey") || "";
  ...
  const registration = vscode2.window.registerWebviewViewProvider("opencodeAgent.sidePanel", provider, {
